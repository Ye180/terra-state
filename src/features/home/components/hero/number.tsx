...
const NumberSection = ({ curentIndex, onClick }: { curentIndex: number; onClick: (index: number) => void }) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            {datas.map((data) => (
                <div className="flex-center gap-2 text-text-primary cursor-pointer" key={data.number} onClick={() => onClick(data.number)}>
                    <span className={cn("text-md lg:text-lg opacity-60", curentIndex === data.number && "opacity-100 text-xl lg:text-3xl")}>
                        {data.number + 1}
                    </span>{" "}
